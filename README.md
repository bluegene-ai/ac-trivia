...
| 排行榜 | 答对记录写库，面板可查、可清空 |
| 管理指令 | `.trivia start [题号] / stop / pause / resume / enable / disable / reload / api / status / stats / chanscan / chanlist` |
...
[AcoreGMPanel](https://github.com/bluegene-ai/AcoreGMPanel) 的 `/trivia` 页提供：
实时状态与控制（走 SOAP `.trivia api`）、节奏/频道/标号/奖励设置、题库 CRUD 与模板导入导出、
奖励预设管理、答对排行。面板通过 `config/trivia.php` 里的 `custom_db_name`（默认 `ac_eluna`）读写上面这些表。
...
```bash
# 138 项行为断言（含内存版假数据库：建表 / 种子导入 / 读库全链路）
